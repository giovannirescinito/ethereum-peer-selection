...
module.exports = async function (callback) {
    ...
    if (paper) {
        ...
        scs = ['MAP','MATRIX']              // scores data structure
...
async function selection() {
    ...
    var quotas = await imp.calculateQuotas(k);
    gas['quotas'] = quotas.receipt.gasUsed
    var allocs = await imp.calculateAllocations();
    gas['allocations'] = allocs.receipt.gasUsed
    var selAll = await imp.selectAllocation(random);
...
function logResults(scoreMatrix, quotas, allocations, selected, winners) {
    logs = winners.receipt.rawLogs;
