...
module.exports = async function (callback) {
    ...
    if (paper) {
        ...
        revPerc = 1
        map = false
        file = "../../results/index_width_optimal/paper_matrix"
        await main()
        map = true
        file = "../../results/index_width_optimal/paper_map"
        await main()
    } else {
        ls = [3, 4, 5]
                        ...
                        for (i4 = 0; i4 < maps.length; i4++) {
                            ...
                            for (i5 = 0; i5 < offchains.length; i5++) {
                                ...
                                for (i6 = 0; i6 < revPercs.length; i6++) {
                                    revPerc = revPercs[i6]
                                    if (checkConditions()) {
                                        file = `../../results/index_width_optimal/l_${l}n_${n}k_${k}m_${m}map_${map}offchain_${offchain}revPerc_${revPerc}`
                                        await main()
                                    } else {
                                        continue
                                    }
